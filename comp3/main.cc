...
int main(int argc, char** argv) {
    ...
    for (size_t col = 0; col < nschools; col++) {
            ...
            for (size_t j = 0; j < tucount; ++j) {
                if (weights[i][j] >= 0) {
                    ...
                    if (j == i) {
                        ...
                        if (noDormMatch && !users[i].dorm.empty() && users[i].dorm == users[j].dorm) {
                            score = -1.f;
                        } else if (j < i && scores[j][i] > 0) {
                            score = scores[j][i];
                        } else {
        ...
        std::vector<std::vector<std::pair<size_t, long long>>> matchesperperson(tucount);
